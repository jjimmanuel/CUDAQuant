...
__global__ void hullWhite(float *results, curandState *states, int numSteps, float dt,
			float a, float sigma, float r0, int numPaths) {
...
int main() {
	...
	// 4. Lauch hull white kernel
	hullWhite<<<blocks, threads_per_block>>>(device_results, device_states, numSteps, dt,
						a, sigma, r0, numPaths);
